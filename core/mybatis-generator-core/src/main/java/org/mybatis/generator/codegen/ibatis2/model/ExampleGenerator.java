...
public class ExampleGenerator extends AbstractJavaGenerator {
    ...
    public List<CompilationUnit> getCompilationUnits() {
        ...
        //----WANGZHIFENG 生成page对象 getIntInstance()
        Field offSet = new Field();
        offSet.setVisibility(JavaVisibility.PUBLIC);
        offSet.setType(FullyQualifiedJavaType.getIntInstance());
        offSet.setName("offSet"); //$NON-NLS-1$
        commentGenerator.addFieldComment(offSet, introspectedTable);
        topLevelClass.addField(offSet);

        Field length = new Field();
        length.setVisibility(JavaVisibility.PUBLIC);
        length.setType(FullyQualifiedJavaType.getIntInstance());
        length.setName("startIndex"); //$NON-NLS-1$
        commentGenerator.addFieldComment(length, introspectedTable);
        topLevelClass.addField(length);
        //---
        method = new Method();
        method.setVisibility(JavaVisibility.PUBLIC);
     ...
     * @param constructor the constructor will be modified to create new
     *            criteria lists for the column
     * @param innerClass the inner class will be modified to add new criteria
    ...
    private List<String> addtypeHandledObjectsAndMethods(IntrospectedColumn introspectedColumn, Method constructor,
        ...
        if (introspectedColumn.getFullyQualifiedJavaType().isPrimitive()) {
            method.addParameter(
                    new Parameter(introspectedColumn.getFullyQualifiedJavaType().getPrimitiveTypeWrapper(), "value")); //$NON-NLS-1$
        } else {
        ...
        if (introspectedColumn.getFullyQualifiedJavaType().isPrimitive()) {
            method.addParameter(
                    new Parameter(introspectedColumn.getFullyQualifiedJavaType().getPrimitiveTypeWrapper(), "value1")); //$NON-NLS-1$
            method.addParameter(
                    new Parameter(introspectedColumn.getFullyQualifiedJavaType().getPrimitiveTypeWrapper(), "value2")); //$NON-NLS-1$
        } else {
     ...
     * 
     * @param introspectedColumn the introspected column
     * @param inMethod if true generates an "in" method, else generates a "not
     *            in" method
